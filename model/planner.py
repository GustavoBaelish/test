...
class MotionPlanner:
    def __init__(self, trajectory_len, feature_len, device, test=False):
        ...
        # set up optimizer
        if test:
            self.optimizer = th.LevenbergMarquardt(objective, th.CholeskyDenseSolver, vectorize=False, max_iterations=50, step_size=0.2, abs_err_tolerance=1e-2)
        else:
            self.optimizer = th.LevenbergMarquardt(objective, th.LUDenseSolver, vectorize=False, max_iterations=2, step_size=0.4, regularize_method=1)
