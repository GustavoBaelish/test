...
def select_future_bestmode(plans, predictions, ground_truth):
...
def select_future(plans, predictions, plan_scores, prediction_scores):
    best_plan_mode = torch.argmax(plan_scores, dim=1)
    best_prediction_mode = torch.argmax(prediction_scores, dim=1)
    plan = torch.stack([plans[i, m] for i, m in enumerate(best_plan_mode)])
    indices = best_prediction_mode[:, None, :, None, None].expand(-1, 1, -1, predictions.shape[3], predictions.shape[4])
    #import ipdb; ipdb.set_trace()
    prediction = torch.gather(predictions, dim=1, index=indices)
...
def check_collision(ego_center_points, neighbor_center_points, sizes):
    # 转成 numpy
    if isinstance(ego_center_points, torch.Tensor):
        ego_center_points = ego_center_points.cpu().numpy()
    if isinstance(neighbor_center_points, torch.Tensor):
        neighbor_center_points = neighbor_center_points.cpu().numpy()
    if isinstance(sizes, torch.Tensor):
        sizes = sizes.cpu().numpy()
    #import ipdb; ipdb.set_trace()
    B, T, _ = ego_center_points.shape
    collision = np.zeros(B, dtype=bool)
    #import ipdb; ipdb.set_trace()

    for b in range(B):
        for t in range(T):
            if check_collision_step(
                ego_center_points[b, t],           # (3,)
                neighbor_center_points[b, :, t],  # (N, 5)
                sizes[b]                          # (N+1, 3)
            ):
                collision[b] = True
                break
...
def check_dynamics(traj):
    ...
    traj = np.array(traj)
    # 计算速度
    v_x = np.diff(traj[:, :, 0], axis=1) / 0.1  # (B, T-1)
    v_y = np.diff(traj[:, :, 1], axis=1) / 0.1
    theta = traj[:, 1:, 2]  # (B, T-1)
    # 计算纵向和横向速度
    lon_speed = v_x * np.cos(theta) + v_y * np.sin(theta)      # (B, T-1)
    lat_speed = v_y * np.cos(theta) - v_x * np.sin(theta)      # (B, T-1)
    # 加速度、横向加速度、jerk
    acc = np.diff(lon_speed, axis=1) / 0.1                    # (B, T-2)
    jerk = np.diff(lon_speed, n=2, axis=1) / 0.01             # (B, T-3)
    lat_acc = np.diff(lat_speed, axis=1) / 0.1                # (B, T-2)
    # 对时间维取平均绝对值
    acc_mean = np.mean(np.abs(acc), axis=1)       # (B,)
    jerk_mean = np.mean(np.abs(jerk), axis=1)     # (B,)
    lat_acc_mean = np.mean(np.abs(lat_acc), axis=1)  # (B,)

    return acc_mean, jerk_mean, lat_acc_mean

def check_traffic(traj, ref_line):
    # 转成 numpy
    if isinstance(traj, torch.Tensor):
        traj = traj.cpu().numpy()
    if isinstance(ref_line, torch.Tensor):
        ref_line = ref_line.cpu().numpy()
    #import ipdb; ipdb.set_trace()
    red_light = []
    off_route = []
    ...
    for b in range(B):
        traj_b = traj[b]  # (T, 3)
        # project to frenet
        distance_to_ref = T.distance.cdist(traj_b[:, :2], ref_line[:, :2])  # (T, N)
        s_ego = np.argmin(distance_to_ref, axis=-1)     # (T,)
        distance_to_route = np.min(distance_to_ref, axis=-1)  # (T,)
        off = np.any(distance_to_route > 5)
        ...
        off_route.append(off)
    #import ipdb; ipdb.set_trace()

    return np.mean(red_light), np.mean(off_route)
...
def check_prediction(trajs, gt):
    ...
    mask = np.not_equal(gt[:, :, :, :2], 0)  # shape: (batch, agent_num, seq_len, 2)
    batch_size, agent_num, seq_len, _ = trajs.shape

    for b in range(batch_size):
        ADE = []
        FDE = []
        for t in range(agent_num):
            if mask[b, t, 0, 0]:  # 只考虑有效轨迹
                error = np.linalg.norm(trajs[b, t, :, :2] - gt[b, t, :, :2], axis=-1)
                error = error * mask[b, t, :, 0]  # mask 无效步
                ADE.append(np.mean(error))
                FDE.append(error[-1])
        ADE_all.append(np.mean(ADE))
        FDE_all.append(np.mean(FDE))
