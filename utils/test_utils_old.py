...
def select_future_bestmode(plans, predictions, plan_scores, prediction_scores):
    ...
    #import ipdb; ipdb.set_trace()
    prediction_scores = prediction_scores*weights_modes.unsqueeze(1)
...
def check_prediction(prediction_trajectories, ground_truth_trajectories, weights):
    #import ipdb; ipdb.set_trace()
    ...
    return plannerADE.item(), plannerFDE.item(), predictorADE.item(), predictorFDE.item(), prediction_loss.item(), plan_score_loss.item(), prediction_score_loss.item(), MR_plan.item(), MR_prediction.item(), diversity_agent_map.item(), diversity_agent_agent.item()
